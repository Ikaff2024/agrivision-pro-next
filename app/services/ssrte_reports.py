...
def build_ficheb_context(profile: SsrteHouseholdProfile) -> dict:
    ...
    return {
        ...
        "producer_ssrte_code": profile.producer_ssrte_code or "—",
        # GPS + heures (B.09a/b/c)
        "gps_start": profile.gps_start or "—",
        "time_start": profile.time_start or "—",
        "time_end": profile.time_end or "—",
        # Type enquete + statut visite (B.10a/b, B.15)
        "survey_type": profile.survey_type or "—",
        "producer_available": profile.producer_available,
        "unavailable_reason": profile.unavailable_reason or "—",
        "visited_person_status": profile.visited_person_status or "—",
        # Travailleurs (B.18b/c/d)
        "external_workers_count": profile.external_workers_count,
        "daily_workers_count": profile.daily_workers_count,
        "non_daily_workers": profile.non_daily_workers or [],
...
def _generate_ficheb_fallback_pdf(context: dict) -> bytes:
    ...
    lines = [
        ...
        f"Code SSRTE producteur : {_pdf_escape(context.get('producer_ssrte_code') or '-')}",
        f"Producteur disponible : {'Oui' if context.get('producer_available') else ('Non' if context.get('producer_available') is False else '-')}",
...
def build_fichec_context(visit: SsrtePlantationVisit) -> dict:
    ...
    return {
        ...
        "section": visit.section or "—",
        "supplier": visit.supplier or "—",
        "sub_prefecture": visit.sub_prefecture or "—",
        "locality": visit.locality or "—",
        "collection_agent_code": visit.collection_agent_code or "—",
        "producer_ssrte_code": visit.producer_ssrte_code or "—",
        # Heures + comptages
        "time_start": visit.time_start or "—",
        "time_end": visit.time_end or "—",
        "adults_count": visit.adults_count,
        "daily_workers_count": visit.daily_workers_count,
        "allow_worker_interview": visit.allow_worker_interview,
        "children_present_count": visit.children_present_count,
        "non_household_children_count": visit.non_household_children_count,
        "non_household_children": visit.non_household_children or [],
        "section_notes": visit.section_notes or {},
...
def _generate_fichec_fallback_pdf(context: dict) -> bytes:
    ...
    lines = [
        ...
        f"Enfants hors menage : {len(context.get('non_household_children') or [])}",
