# Guide d'utilisation AgriVision Pro — Sommaire

> Ce guide explique l'application **module par module**, pas à pas, avec des mots simples.
> Aucune compétence technique n'est nécessaire. Si vous savez ouvrir un site web et cliquer,
> vous savez utiliser AgriVision Pro.

## Par où commencer ?
> 👉 Nouveau sur la plateforme ? Suivez le **[Scénario de prise en main « du planteur au lot conforme »](15_prise_en_main.md)** :
> un parcours guidé de bout en bout (avec exercices) qui couvre l'essentiel en une seule séance.

## Comment lire ce guide
- Chaque fichier traite **un sujet**. Lisez d'abord le **Démarrage**, puis le module qui vous intéresse.
- Dans chaque fiche : **À quoi ça sert**, **Qui peut l'utiliser**, **Pas à pas**, **Astuces**, **En cas de problème**.
- Les boutons et menus sont écrits **en gras** (ex. cliquez sur **Enregistrer**).

## Sommaire — les 4 piliers d'AgriVision Pro

> 🌱 **Produire** · 📦 **Tracer** · 🌍 **Protéger** · 📊 **Piloter** — plus un socle de mise en route.
> Les numéros renvoient aux fiches détaillées (aussi accessibles dans le menu de gauche).

### 🌱 Produire — gérer les exploitations et leur performance
2. [Plantations & Producteurs](02_plantations_producteurs.md)
3. [Diagnostic, Carte & délimitation, Satellite](03_diagnostic_carte_satellite.md)
4. [Agroforesterie & Récoltes](04_agroforesterie_recoltes.md)
13. [Jumeau de parcelle & Parcelles à risque](13_jumeau_parcelle.md)

### 📦 Tracer — du champ jusqu'à l'acheteur
5. [Achats, Traçabilité des lots & Certification](05_achats_lots_certification.md)

### 🌍 Protéger — conformité & durabilité *(le différenciateur)*
6. [CacaoGuard, Protection de l'enfant & Fiches SSRTE](06_cacaoguard_ssrte.md)
   — 🎓 [Scénario guidé « Protection de l'enfant »](16_protection_enfant.md) (du terrain à la due diligence)
7. [EUDR & Conformité](07_eudr_conformite.md)
8. [Revenu vital](08_farmforce_revenu_vital.md)
9. [Monitoring, Remédiation, Signalements, Formation](09_suivi_terrain.md) — l'exécution terrain de la protection (+ **travailler sans réseau**, §9.F)

### 📊 Piloter — décider et prouver
11. [Dashboard, Direction & Rapports](11_dashboard_direction_rapports.md)

### ⚙️ Socle — mise en route
1. [Démarrage : connexion, mot de passe, navigation, rôles](01_demarrage.md)
10. [Import d'un registre Excel](10_import_registre.md)
12. [Administration](12_administration_proprietaire.md)

## Les 3 niveaux d'utilisateur (résumé)
| Rôle | Ce qu'il peut faire |
|---|---|
| **Administrateur** | Tout dans sa coopérative : créer/supprimer, gérer les membres, importer, changer les mots de passe. |
| **Agronome** | Saisir et consulter (diagnostics, fiches, achats, lots…), générer les rapports. |
| **Technicien** | Travail de terrain : voit surtout ses parcelles attribuées, saisit visites/achats. |

## Le principe en une phrase
AgriVision Pro relie **le producteur → sa parcelle → ses récoltes → le lot vendu**, tout en assurant
la **conformité** (EUDR, travail des enfants) et la **durabilité** (revenu vital, agroforesterie).
