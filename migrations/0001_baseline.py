...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("cooperative_id", sa.Integer(),
                  sa.ForeignKey("cooperatives.id"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True),
                  server_default=sa.func.now(), nullable=False),
    )
    ...
    op.create_table(
        "diagnostics",
        sa.Column("id", sa.Integer(), primary_key=True, index=True),
        sa.Column("plantation_id", sa.Integer(),
                  sa.ForeignKey("plantations.id"), nullable=False),
